...
from Kazu import app, CMD_HELP
from Kazu.helpers.cmd import *
from Kazu.helpers.PyroHelpers import ReplyCheck
from Kazu.helpers.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "Wildan-UserBot Plugins"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• @musicsupport_dan × @Disney_storeDan •",
